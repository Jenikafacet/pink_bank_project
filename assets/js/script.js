...
fetch(
    ...
    .then((data) => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            let date = new Date();
            let newsDate = new Date(data.articles[i].publishedAt.split("T")[0])
            if (date.getDate() === newsDate.getDate()) {
                ...
                newsItemInfo.textContent = data.articles[i].publishedAt.split("T")[0] + " " + data.articles[i].publishedAt.split("T")[1].slice(0, -4) + ", " + data.articles[i].source.name;
            }
